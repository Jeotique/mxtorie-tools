...
module.exports = {
    ...
    run: async(client, guild, member) => {
        ...
        if(!channel) return
        const embedData = client.db.get(`welcomeembedunknow_${guild.id}`) || client.defaultVariables.welcomeEmbedUnknow
        if(client.db.get(`welcomeunknow_isembed_${guild.id}`)){
            ...
            return channel.send({embeds: [embed]}).then(msg => {
                let deleteAfter = client.db.get(`welcomeunknow_deleteafter_${guild.id}`)
            ...
            let brutMessage = client.db.get(`welcomemessageunknow_${guild.id}`) || client.defaultVariables.welcomeMessage
            Object.keys(client.variablesReplace.joinUnknow).map(key => {
