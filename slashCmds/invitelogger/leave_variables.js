...
module.exports = {
    ...
    run: async(client, interaction)=>{
        ...
        let variablesObject;
        switch(type){
            case 'know_invite': variablesObject = client.variablesReplace.leaveNormal; break;
            case 'vanity': variablesObject = client.variablesReplace.leaveVanity; break;
            case 'unknow': variablesObject = client.variablesReplace.leaveUnknow; break;
            case 'himself': variablesObject = client.variablesReplace.leaveUnknow; break;
        }
        let invites = await interaction.guild.invites.fetch().catch(e=>{})
        let invite = invites?.first()
        invite.data = client.db.get('invitesStats_' + interaction.guildId + '_' + invite.inviterId)
